...
class App(tk.Tk):
    def __init__(self):
        ...
        # Контейнер для регулируемых областей
        container = tk.PanedWindow(main_frame, orient=tk.VERTICAL)
        container.pack(fill=tk.BOTH, expand=True)

        # Upper Frame: Left side with TreeView
        upper_frame = tk.Frame(container)
        container.add(upper_frame)

        # Lower Frame: Text area
        lower_frame = tk.Frame(container)
        container.add(lower_frame)

        # Левая сторона: дерево с устройствами
        tree_frame = tk.Frame(upper_frame)
        tree_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

        # Заголовок дерева
        title_label = tk.Label(tree_frame, text="Обнаруженные уникальные адреса", font=("TkDefaultFont", 10, 'bold'))
        ...
        self.tree.heading('#2', text='Vendor', anchor='center', command=lambda: self.sort_column("#2"))
        self.tree.heading('#3', text='RSSI', anchor='center', command=lambda: self.sort_column("#3"))
        self.tree.heading('#4', text='Last Seen', anchor='center', command=lambda: self.sort_column("#4"))
        ...
        # Правая сторона: панель с кнопками
